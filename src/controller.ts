...
export const explore = async (ns: NS) => {
  ...
  servers["home"].maxRam = Math.max(
    0,
    servers["home"].maxRam - settings.homeRamReserved,
  );
...
async function expFarm(ns: NS, hackingNodes: ServerNode[], stopTime: number) {
  while (new Date().getTime() < stopTime) {
    ...
    if (!DEBUG.dryrun) {
      for (const node of activeNodes) {
        executeAttackAction(
          ...
          "weaken.js",
          node.host,
          settings.expFarmTarget,
          node.availableCycles,
          0,
        );
      }
    }
    ...
    if (DEBUG.expFarm) {
      ns.tprint(
        `Exp farming with ${totalCycles} cycles on ${activeNodes.length} nodes.`,
      );
    }
...
export function executeAttackAction(
  ...
  if (!DEBUG.dryrun) {
    ...
    if (retval === 0) {
      const serverRam = ns.getServerMaxRam(attacker) -
        ns.getServerUsedRam(attacker);
      const threadsThatFit = Math.floor(serverRam / 1.75);
      throw new Error(
        `Failed to execute ${action} on ${attacker} with ${threads} threads, node has space for ${threadsThatFit} threads!`,
      );
    }
  }
}
...
const locateTargets = (
  ...
  const topServers = potentialTargets.slice(
    0,
    Math.min(settings.targetServerCount, potentialTargets.length),
  );
...
export const hackThreadsNeededToSteal = (
  ns: NS,
  target: string,
  percent: number,
): number => {
...
export async function main(ns: NS) {
  const attack = async (hackingNodes: ServerNode[], targets: ServerNode[]) => {
    ...
    const calculatePrepare = (
      ...
      const additionalWeakensToPerform = Math.min(
        additionalWeakenCount,
        cyclesAvailable,
      );

      const total = weakensToPerform + growToPerform +
        additionalWeakensToPerform;
    ...
    const debugAttackActionFormat =
      "DEBUG: attacker %s; target %s; action %s, count %d; need %d; left: node %d; total %d;";
    ...
    for (const target of targets) {
      ...
      for (const node of hackingNodes) {
        ...
        if (cyclesNeeded.weaken > 0 && node.availableCycles > 0) {
          ...
          if (DEBUG.attackActions) {
            ns.tprintf(
              debugAttackActionFormat,
              node.host,
              target.host,
              "weaken",
              count,
        ...
        if (cyclesNeeded.grow > 0 && node.availableCycles > 0) {
          ...
          executeAttackAction(
            ...
            target.host,
            count,
            actionTimes.growDelay,
          );
          cycles -= count;
          node.availableCycles -= count;
          cyclesNeeded.grow -= count;

          longestWait = Math.max(
            longestWait,
            actionTimes.growDelay + actionTimes.grow,
          );
        ...
        if (cyclesNeeded.additionalWeakens > 0 && node.availableCycles > 0) {
          const count = Math.min(
            cyclesNeeded.additionalWeakens,
            node.availableCycles,
          );
          executeAttackAction(
            ns,
            ...
            target.host,
            count,
            actionTimes.additionalWeakenDelay,
          );
          cycles -= count;
          node.availableCycles -= count;
          ...
          longestWait = Math.max(
            longestWait,
            actionTimes.additionalWeakenDelay + actionTimes.weaken,
          );
    ...
    ns.tprint(
      `Longest wait: ${ns.tFormat(longestWait)}. Remaining cycles: ${cycles}.`,
    );
  ...
  while (true) {
    ...
    const targets = locateTargets(ns, servers, capacity).map(({ hostname }) =>
      servers[hostname]
    );
...
function createHWGWBatch(ns: NS, target: ServerNode): HWGWBatch {
  const hackCycles = hackThreadsNeededToSteal(
    ns,
    target.host,
